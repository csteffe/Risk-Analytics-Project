...
The maxima identified within each clusters will then be fit to the GPD.
...
[ANOTHER VERSION OF DECLUSTERING PLOT]
```{r decluster, echo=F, warning=F, message=F}
# Decluster 
decluster <- extRemes::decluster(niveau$Wert, 
                                 threshold = 327)
...
[TRIED TO CONVERT DAILY DATA INTO YEARLY]
```{r return levels with GDP, echo=F, warning=F, message=F}
...
Advantages:
The Block Maxima method gives more efficient results when less exceedances are observed. 
This method may also be preferable when the observations are not exactly independant and identically distributed. For example, when there is seasonal periodicity in case of yearly maxima or, when there is short range dependence that plays a role within blocks but not between blocks.

Drawbacks: 
BM is less flexible than POT since in practice it might be hard to change the block size, while it is easy in POT to move the threshold continuously.
Moreover, there are less possibility of very detailed modeling of temporal distribution of extremes than with POT.
