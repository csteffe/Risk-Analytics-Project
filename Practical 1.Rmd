...
The **Peak-Over-Threshold** method provides a model for independent exceedances (i.e. extreme values) above a large threshold. Therefore, it identifies values that are **high enough** to be above a designated threshold $\mu$, above which we consider are the extremes (i.e. highest values). In order to determine an optimal threshold, we apply the **MRL-plot** and then look at the distribution of the exceedances. 

The value of $u$ from which the plot becomes approximately linear can generally be selected as the optimal threshold. [not sure I understand this sentence] Therefore, to model the high water levels, we first proceed to an MRL-plot to choose the optimal threshold $u$ and then, we use the Peaks-over-Threshold method [we look at the distribution of the exceedances to assess their probability of occurrence].
...
The mean excess function $e(u) = E(X - u| X > u)$ is the expected value of the difference between the random variable (i.e. exceedances) and the high threshold (i.e. $u$), knowing that the exceedance is greater than the threshold . [check if translation from algebric expression is correct] And we are interested in finding the distribution of the daily water level maxima that are above the threshold $u$, which is the excess distribution function $F_u(x) = P(X - u \leq x | X > u)$. 
...
[Notes for ourselves in case of a question: "If we take the threshold to be so low that most of the data are exceedances then the analysis will only be valid if the raw data came from a generalized Pareto distribution (which will rarely be the case)"]

The below **Mean Residual Life Plot** indicates for each potential threshold $\mu$ its mean excess. As mentioned previously, the value of $u$ from which the plot becomes approximately linear can generally be selected as the optimal threshold. The dashed curves represent the confidence intervals of the mean excesses which help assess its type (i.e. constant, linear). Therefore, we decide to choose a threshold $u = 327\ [m.a.s.l]$ form which the curve seems to become more linear.

From now on, daily water level maxima above $327\ [m.a.s.l]$ are considered exceedances/extreme values/outliers [quel est le meilleur terme?]. 

[Notes for ourselves: below 327 a threshold would be too low for the assumptions of the extreme value approach to be valid and above 328 would be too high to obtain insghtful results because of too few data]
...
The below plot shows the results of the Peak-Over-Treshol method. The red line represents the designated appropriate threshold $\mu = 327\ [m.a.s.l]$ and the red dots are the extreme daily water level maxima recorded, considered as exceedances. 
...
The Peak-over-Threshold method needs the threshold exceedances (i.e. data points above it) to be independant. As there may be correlation/autocorrelation in the daily water level values, we decided to decluster these data above the chosen threshold (here 327 meter above the sea) to make them independant.
...
Extreme Value theory suggests that GPD is a *natural approximation* of the excess distribution function $F_u(x)$ where $x$ is the excess, so $F_u(x) \approx G_{\xi, \beta}(x)$. [maybe add parameters conditions]

Under the assumption that exceedances are **independent** (i.e. no auto-correlation?), **identically distributed** (i.e. are random variables) and their distribution is **asymptotic** (i.e. ?), we fit the GDP model on the daily water level exceedances.  
