...
add_lines(x = ~date, 
          ...
          name = "daily count",
          yaxis = "y2") %>%
  layout(title = "daily count for 2016 summery Olympics",
         yaxis2 = hon_yaxis) %>% 
...
u <- c(60, 70, 125, 200, 100, 700, 100, 50, 35, 100, 25, 30)
...
for (i in 1:ncol(data99)){
 ...
 # compute 99 quantile
 quantile99 <- abs(quantile(ts_type$`daily count modified` , 0.01))
 ...
 uncertainty <- fpot(ts_type$`daily count modified`, threshold = u[i], mper = quantile99)
 
 # need to find which element of fpot to keep for uncertainty after the dolar
 #data99[2,i] <- uncertainty$
}
...
```{r}
# measure of uncertainty
#Some analytical formulas exist (e.g. see mper= argument in evd::fpot).
```
