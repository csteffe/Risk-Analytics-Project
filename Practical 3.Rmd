...
for (i in levels(type)) {
  ...
  for (j in length(levels(pageviews_ts$type))) {
    ...
    modified_ts$dailycount <- "daily count modified"
  }
}
...
ts %>%
  ...
  geom_line(mapping = aes(y = dailycount, colour = "dailycount")) +
  geom_line(mapping = aes(y = `modified daily count`, colour = "modified daily count"))
...
- Estimate the 99%-quantile of each series and give a corresponding measure of uncertainty
